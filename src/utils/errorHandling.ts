 ...
 */
export const isUserManagementError = (error: unknown): error is UserManagementError => {
    return (
        error instanceof Error &&
        'code' in error &&
        ['PERMISSION_DENIED', 'USER_NOT_FOUND', 'EMAIL_EXISTS', 'INVALID_DATA'].includes(
            (error as UserManagementError).code
        )
    );
};
...
export const handlePermissionError = (
    ...
    const shouldRedirect = isPermissionError(error) &&
