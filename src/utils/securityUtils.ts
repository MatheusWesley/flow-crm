...
export const isValidEmail = (email: string): boolean => {
    ...
    return emailRegex.test(email) && hasValidLength && hasNoScripts && hasNoSqlInjection;
};
...
export const logSecurityEvent = (
    ...
    try {
        const existingLogs = JSON.parse(localStorage.getItem('flowcrm_security_logs') || '[]');
...
export const detectSecurityThreats = (input: string): string[] => {
    ...
    // XSS detection
    if (/<script|javascript:|data:|vbscript:|onload|onerror|onclick/i.test(input)) {
        threats.push('Potential XSS attempt detected');
    }

    // SQL injection detection
    if (/('|(--)|;|\/\*|\*\/|xp_|sp_|exec|union|select|insert|update|delete|drop)/i.test(input)) {
