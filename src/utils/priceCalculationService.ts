...
class PriceCalculationService {
	private config: PriceCalculationConfig;

	constructor(config?: Partial<PriceCalculationConfig>) {
		this.config = {
	...
	calculateSuggestedPrice(
		...
		const clampedMargin = Math.max(
			...
			Math.min(this.config.maxMargin, margin),
		);
	 ...
	 */
	updateConfig(newConfig: Partial<PriceCalculationConfig>): void {
		this.config = {
			...this.config,
			...newConfig,
		};
	 ...
	 */
	getConfig(): PriceCalculationConfig {
		return { ...this.config };
	}
}
