...
export const validatePermissions = (permissions: UserPermissions): boolean => {
    try {
        ...
        const requiredModules = ['products', 'customers', 'reports', 'paymentMethods', 'userManagement'];
        for (const module of requiredModules) {
            if (typeof permissions.modules[module as ModulePermission] !== 'boolean') {
                return false;
            }
        }
        ...
        for (const perm of requiredPresalesPerms) {
            if (typeof permissions.presales[perm as PresalePermission] !== 'boolean') {
                return false;
            }
        }
 ...
 */
export const mergeWithDefaults = (userType: UserType, permissions?: Partial<UserPermissions>): UserPermissions => {
 ...
 */
export const filterPermissionsByUserType = (userType: UserType, permissions: UserPermissions): UserPermissions => {
    if (userType === 'admin') {
        return permissions;
    }
 ...
 */
export const getGroupedPermissions = (userType: UserType): Record<string, string[]> => {
 ...
 */
export const canPerformPresaleAction = (user: User, action: 'create' | 'viewOwn' | 'viewAll', presaleUserId?: string): boolean => {
    if (!user || !user.permissions) {
        return false;
    }
    ...
    switch (action) {
        ...
        case 'viewAll':
            ...
            return user.permissions.presales.canViewAll ||
