...
export class UserService implements UserManagementService {
    ...
    async createUser(userData: CreateUserRequest): Promise<User> {
        try {
            ...
            throw new Error(error.response?.data?.error?.message || 'Falha ao criar usuário');
        }
    }
    ...
    async updateUser(userId: string, userData: UpdateUserRequest): Promise<User> {
        try {
            ...
            throw new Error(error.response?.data?.error?.message || 'Falha ao atualizar usuário');
        }
    }
    ...
    async deleteUser(userId: string): Promise<void> {
        try {
            ...
            if (error.response?.data?.error?.code === 'CANNOT_DELETE_SELF') {
                const userError = new Error('Não é possível excluir sua própria conta') as any;
                userError.code = 'PERMISSION_DENIED';
                throw userError;
            }

            throw new Error(error.response?.data?.error?.message || 'Falha ao excluir usuário');
        }
    }
    ...
    async updateUserPermissions(userId: string, permissions: UserPermissions): Promise<void> {
        try {
            ...
            throw new Error(error.response?.data?.error?.message || 'Falha ao atualizar permissões do usuário');
        }
    }
 ...
 */
export const getDefaultPermissions = (userType: 'admin' | 'employee'): UserPermissions => {
