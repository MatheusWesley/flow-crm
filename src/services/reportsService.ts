...
class ReportsService {
    ...
    private async withRetry<T>(
        ...
        try {
            ...
            console.warn(`Request failed, retrying in ${delay}ms... (${retries} attempts left)`, error.message);
    ...
    async getPaymentMethodsReport(
        ...
        try {
            ...
            const response = await this.withRetry(async () => {
                return await httpClient.get<{ success: boolean; data: PaymentMethodReportData[]; message: string }>(url);
            });
    ...
    async getReportSummary(filters?: ReportFilters): Promise<ReportSummary> {
        try {
            ...
            const response = await this.withRetry(async () => {
                return await httpClient.get<{ success: boolean; data: ReportSummary; message: string }>(url);
            });
            ...
            const requiredFields = ['totalAmount', 'totalSalesCount', 'totalConvertedPresales'];
