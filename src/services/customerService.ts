...
export class CustomerService {
    ...
    async deleteCustomer(id: string): Promise<void> {
        try {
            ...
            if (error.response?.data?.message?.includes('foreign key constraint') ||
                ...
                throw new Error('Não é possível excluir este cliente pois ele possui pré-vendas associadas.');
            }
            throw error;
        }
    }
     ...
     */
    async searchCustomers(query: string, params?: Omit<CustomerQueryParams, 'search'>): Promise<PaginatedResponse<Customer>> {
