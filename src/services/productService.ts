    ...
    async deleteProduct(id: string): Promise<void> {
        try {
            ...
            if (error.status === 409 || error.code === 'CONFLICT') {
                throw new Error('Não é possível excluir este produto pois ele possui movimentações (pré-vendas) associadas.');
            }
            ...
            if (error.message?.includes('delete from "products"') ||
            ...
            throw new Error(error.message || 'Erro ao excluir produto. Tente novamente.');
        }
    }
