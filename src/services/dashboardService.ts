import type { Customer, Product, PreSale, PaginatedResponse } from '../types/api';
...
class DashboardService {
    ...
    async getDashboardMetrics(): Promise<DashboardMetrics> {
        try {
            ...
            const todaySales = presales.filter((presale: any) => {
                ...
                return presaleDateString === todayDateString && presale.status === 'converted';
            });



            const salesToday = todaySales.reduce((sum: number, presale: any) => sum + Number(presale.total), 0);
            ...
            const currentMonthSales = presales.filter((presale: any) => {
                ...
                return presaleMonth === currentMonth && presaleYear === currentYear && presale.status === 'converted';
            });

            const monthlyRevenue = currentMonthSales.reduce((sum: number, presale: any) => sum + Number(presale.total), 0);
            ...
            const lastMonthSales = presales.filter((presale: any) => {
                ...
                const presaleMonth = presaleDate.getMonth() + 1;
                const presaleYear = presaleDate.getFullYear();
                return presaleMonth === lastMonth && presaleYear === lastMonthYear && presale.status === 'converted';
            });

            const lastMonthRevenue = lastMonthSales.reduce((sum: number, presale: any) => sum + Number(presale.total), 0);
            ...
            const activeCustomerIds = new Set(
                presales
                    .filter((presale: any) => {
                        const presaleDateString = new Date(presale.createdAt).toISOString().split('T')[0];
        ...
        try {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const daySales = presales.filter((presale: any) => {
                    const presaleDate = new Date(presale.createdAt);
                    const presaleDateString = presaleDate.toISOString().split('T')[0];
                    return presaleDateString === dateString && presale.status === 'converted';
                });

                const dailyRevenue = daySales.reduce((sum: number, presale: any) => sum + Number(presale.total), 0);
    ...
    async getInventoryAlerts(): Promise<InventoryAlert[]> {
        try {
            ...
            const lowStockProducts = products.filter((product: any) => product.stock < 15);
