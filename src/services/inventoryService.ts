...
export interface InventoryError extends Error {
    code: 'NETWORK_ERROR' | 'VALIDATION_ERROR' | 'NOT_FOUND' | 'INSUFFICIENT_STOCK' | 'CONFLICT' | 'UNAUTHORIZED' | 'SERVER_ERROR' | 'UNKNOWN_ERROR';
    status?: number;
    details?: any;
}
...
class InventoryService {
    ...
    private transformApiError(error: any, operation: string): InventoryError {
        ...
        switch (status) {
            case 400:
                ...
                if (validationMessage.includes('insufficient stock') || validationMessage.includes('negative stock')) {
                    return this.createInventoryError(
                ...
                return this.createInventoryError(
                    validationMessage || 'Invalid request data. Please check your input and try again.',
                    'VALIDATION_ERROR',
                    status,
                    errorData
                );
     ...
     */
    private validateStockAdjustment(productId: string, adjustment: StockAdjustmentRequest): void {
        if (!productId?.trim()) {
    ...
    async getStockHistory(filters: StockHistoryFilters = {}): Promise<{
        ...
        return this.executeWithRetry(
            async () => {
                ...
                if (filters.productId?.trim()) params.append('productId', filters.productId.trim());
                ...
                const response = await httpClient.get<PaginatedResponse<StockAdjustment>>(url);
    ...
    private validateStockHistoryFilters(filters: StockHistoryFilters): void {
        ...
        if (filters.limit && (filters.limit < 1 || filters.limit > 100 || !Number.isInteger(filters.limit))) {
            throw this.createInventoryError(
        ...
        if (filters.adjustmentType && !['add', 'remove'].includes(filters.adjustmentType)) {
            throw this.createInventoryError(
    ...
    async getProduct(productId: string): Promise<Product> {
        ...
        return this.executeWithRetry(
            async () => {
                const response = await httpClient.get<ApiResponse<Product>>(`${this.baseUrl}/products/${productId.trim()}`);
                return this.transformProduct(response.data);
            },
            'getProduct',
            {
