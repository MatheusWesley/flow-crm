...
import type { CreatePaymentMethodRequest, UpdatePaymentMethodRequest } from '../types/api';
...
export class PaymentMethodService {
    ...
    async getAll(): Promise<PaymentMethod[]> {
        try {
            const response = await httpClient.get<{ success: boolean; data: PaymentMethod[] }>(this.baseUrl);
    ...
    async getById(id: string): Promise<PaymentMethod> {
        try {
            const response = await httpClient.get<{ success: boolean; data: PaymentMethod }>(`${this.baseUrl}/${id}`);
            return response.data;
        } catch (error) {
    ...
    async create(data: CreatePaymentMethodRequest): Promise<PaymentMethod> {
        try {
            const response = await httpClient.post<{ success: boolean; data: PaymentMethod }>(this.baseUrl, data);
            return response.data;
        } catch (error) {
    ...
    async update(id: string, data: UpdatePaymentMethodRequest): Promise<PaymentMethod> {
        try {
            const response = await httpClient.put<{ success: boolean; data: PaymentMethod }>(`${this.baseUrl}/${id}`, data);
            return response.data;
        } catch (error) {
    ...
    async delete(id: string): Promise<boolean> {
        try {
            ...
            if (error instanceof Error && error.message.includes('409')) {
                throw new Error('Esta forma de pagamento está sendo usada e não pode ser excluída.');
            }
    ...
    async toggleActive(id: string, isActive: boolean): Promise<PaymentMethod> {
        try {
            const response = await httpClient.patch<{ success: boolean; data: PaymentMethod }>(`${this.baseUrl}/${id}/status`, { isActive });
            return response.data;
        } catch (error) {
