import { Package, History, Search, RefreshCw, Plus, Minus, X } from 'lucide-react';
import type React from 'react';
import { useState, useEffect, useCallback } from 'react';
import toastService, { TOAST_MESSAGES } from '../../../services/ToastService';
import inventoryService, { type StockAdjustmentRequest, type StockAdjustment as ApiStockAdjustment, type InventoryError } from '../../../services/inventoryService';
...
const InventoryPage: React.FC = () => {
		...
		try {
			switch (type) {
				...
				case 'adjust':
					...
					toastService.warning('Ajuste de estoque não pode ser repetido automaticamente. Verifique o estoque atual e tente novamente se necessário.');
					break;
				case 'history':
	...
	const executeWithRetry = async <T,>(
		...
		try {
			...
			if (inventoryError.code === 'VALIDATION_ERROR' ||
			...
			const shouldRetry = operationType !== 'adjust' &&
				currentAttempt < maxRetries &&
				(inventoryError.code === 'NETWORK_ERROR' || inventoryError.code === 'SERVER_ERROR');

			if (shouldRetry) {
				currentAttempt++;
				setRetryAttempts(prev => ({ ...prev, [operationKey]: currentAttempt }));
				...
				toastService.info(`${TOAST_MESSAGES.inventory.retrying} (Tentativa ${currentAttempt}/${maxRetries})`, {
					autoClose: 2000
				});
				...
				// Recursive retry
				return executeWithRetry(operation, operationType, operationData, maxRetries);
			}
	...
	const validateHistoryFilter = (field: keyof typeof historyFilters, value: string, currentFilters: typeof historyFilters): string => {
		switch (field) {
			case 'productCode':
			...
			case 'endDate':
				if (value && currentFilters.startDate && value < currentFilters.startDate) {
	...
	const handleFilterChange = (field: keyof typeof historyFilters, value: string) => {
	...
	const loadStockHistory = async (page: number = 1, resetData: boolean = true) => {
		if (!isOnline) {
		...
		if (resetData && page === 1) {
			toastService.info(TOAST_MESSAGES.inventory.loadingHistory, { autoClose: 2000 });
		}
		...
		try {
			const filters = {
				...
				...(historyFilters.productCode && { productCode: historyFilters.productCode }),
				...(historyFilters.adjustmentType && { adjustmentType: historyFilters.adjustmentType }),
	...
	const validateForm = (): boolean => {
		...
		} else if (!/^[a-zA-Z0-9\s\-.,!?()áéíóúàèìòùâêîôûãõçÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕÇ]+$/.test(formData.reason.trim())) {
			errors.reason = 'Motivo contém caracteres inválidos';
		}

		// Check for insufficient stock on remove operations
		if (selectedProduct && formData.adjustmentType === 'remove' && formData.quantity) {
			const quantity = Number(formData.quantity);
		...
		if (selectedProduct && formData.adjustmentType === 'add' && formData.quantity) {
			const quantity = Number(formData.quantity);
			const newStock = selectedProduct.stock + quantity;
			if (newStock > 999999) {
				errors.quantity = 'O estoque resultante seria muito alto (máximo: 999.999)';
			}
		}
	...
	const validateField = (field: string, value: string): string => {
		switch (field) {
			case 'productCode':
				...
				if (value.trim().length < 2) return 'Código deve ter pelo menos 2 caracteres';
				if (value.trim().length > 50) return 'Código deve ter no máximo 50 caracteres';
				if (value.trim() && !selectedProduct && !isSearchingProduct) return TOAST_MESSAGES.inventory.productNotFound;
				return '';
			...
			case 'quantity':
				...
				if (!Number.isInteger(quantity)) return 'Quantidade deve ser um número inteiro';

				// Check stock limits for remove operations
				if (selectedProduct && formData.adjustmentType === 'remove' && quantity > selectedProduct.stock) {
			...
			case 'reason':
				...
				if (value.trim().length < 3) return TOAST_MESSAGES.inventory.reasonTooShort;
				if (value.trim().length > 500) return TOAST_MESSAGES.inventory.reasonTooLong;
				if (!/^[a-zA-Z0-9\s\-.,!?()áéíóúàèìòùâêîôûãõçÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕÇ]+$/.test(value.trim())) {
	...
	const refreshSelectedProduct = async () => {
		...
		try {
			const refreshedProduct = await inventoryService.getProduct(selectedProduct.id);
			setSelectedProduct(refreshedProduct);

			// Show warning if stock changed
			if (refreshedProduct.stock !== selectedProduct.stock) {
				toastService.info(`Estoque atualizado: ${refreshedProduct.stock} ${refreshedProduct.unit}`);
			}
		} catch (error) {
	...
	useEffect(() => {
		const handleOnline = () => {
			...
			if (lastFailedOperation && Date.now() - lastFailedOperation.timestamp < 300000) {
		...
		const handleOffline = () => {
			setIsOnline(false);
			toastService.warning(TOAST_MESSAGES.inventory.offlineMode, { autoClose: false });
		};
	...
	const getFilteredAndSortedProducts = useCallback(() => {
		...
		if (filterByStock === 'in_stock') {
			...
			filteredProducts = filteredProducts.filter(product => product.stock <= 10); // Assuming low stock is <= 10
		}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		toastService.info(TOAST_MESSAGES.inventory.processingAdjustment, { autoClose: 2000 });
		...
		try {
			...
			const updatedProduct = await inventoryService.adjustStock(selectedProduct!.id, adjustmentRequest);
			...
			if (formData.adjustmentType === 'remove' &&
				...
				toastService.warning('O estoque foi alterado por outro usuário. Verifique o valor atual.');
			}
			...
			switch (inventoryError.code) {
				...
				case 'VALIDATION_ERROR':
					errorMessage = inventoryError.message || TOAST_MESSAGES.inventory.validationError;
					break;
				case 'NETWORK_ERROR':
					errorMessage = TOAST_MESSAGES.inventory.networkError;
					break;
				...
				case 'SERVER_ERROR':
					...
					break;
				default:
					errorMessage = inventoryError.message || TOAST_MESSAGES.inventory.serverError;
			}
							...
							<div className="space-y-6">
									...
									<div>
										...
										<div className="flex space-x-3">
											<button
												...
												className={`flex-1 flex items-center justify-center px-3 py-2 rounded-md border transition-all duration-200 ${formData.adjustmentType === 'add'
											...
											<button
												...
												className={`flex-1 flex items-center justify-center px-3 py-2 rounded-md border transition-all duration-200 ${formData.adjustmentType === 'remove'
								...
								<div>
									...
									<div className="flex justify-end mt-1">
										<span
											className={`text-xs font-semibold px-2 py-0.5 rounded-full ${formData.reason.length >= 450
									...
									<div className="flex items-center justify-between">
										<div className="flex-1">
											<div className="flex items-center space-x-3">
												<div className={`p-2 rounded-full ${adjustment.adjustmentType === 'add'
											...
											<div className="mt-2 ml-11">
												<p className="text-sm text-gray-700">
													<span className="font-medium">Quantidade:</span> {adjustment.quantity}
												</p>
												<p className="text-sm text-gray-700">
										...
										<div className="text-right">
											...
											<p className="text-xs text-gray-400">
												{new Date(adjustment.createdAt).toLocaleTimeString('pt-BR')}
											</p>
										</div>
									</div>
								</div>
	...
	return (
		<div className="p-6">
			...
			{showProductSearch && (
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowProductSearch(false)}>
					<div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
						...
						<div className="p-6 overflow-y-auto max-h-96">
							...
							{!isLoadingProducts && getFilteredAndSortedProducts().length === 0 && (
								<div className="text-center py-8">
