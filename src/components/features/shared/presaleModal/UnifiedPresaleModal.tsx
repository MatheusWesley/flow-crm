...
const UnifiedPresaleModal: React.FC<UnifiedPresaleModalProps> = ({
	...
	const handleProductDescriptionChange = (description: string) => {
		...
		if (matchingProduct && description.length > 2) {
			const convertedProduct = {
				...matchingProduct,
				purchasePrice: typeof matchingProduct.purchasePrice === 'string' ? Number(matchingProduct.purchasePrice) : matchingProduct.purchasePrice,
				salePrice: typeof matchingProduct.salePrice === 'string' ? Number(matchingProduct.salePrice) : matchingProduct.salePrice,
				createdAt: typeof matchingProduct.createdAt === 'string' ? new Date(matchingProduct.createdAt) : matchingProduct.createdAt,
				updatedAt: typeof matchingProduct.updatedAt === 'string' ? new Date(matchingProduct.updatedAt) : matchingProduct.updatedAt,
			} as any;
	...
	const handleSubmitForm = (e: React.FormEvent) => {
		...
		const presaleData: Omit<PreSale, 'id' | 'createdAt' | 'updatedAt'> = {
			customer: {
				...selectedCustomer,
				createdAt: typeof selectedCustomer.createdAt === 'string' ? new Date(selectedCustomer.createdAt) : selectedCustomer.createdAt,
				updatedAt: typeof selectedCustomer.updatedAt === 'string' ? new Date(selectedCustomer.updatedAt) : selectedCustomer.updatedAt,
			},
			items: formItems.map((item, index) => ({
				id: `item-${index}`,
				product: {
					...item.product,
					purchasePrice: typeof item.product.purchasePrice === 'string' ? Number(item.product.purchasePrice) : item.product.purchasePrice,
					salePrice: typeof item.product.salePrice === 'string' ? Number(item.product.salePrice) : item.product.salePrice,
				},
				quantity: item.quantity,
				unitPrice: item.unitPrice,
									...
									>
										<div className="flex items-center gap-3 flex-1 min-w-0">
											...
											<div className="flex-1 min-w-0">
												...
												<p className="text-xs text-gray-500">
													{item.product.code} • {item.quantity} {item.product.unit || 'un'} × R$ {item.unitPrice.toFixed(2)}
												</p>
											</div>
										</div>
						...
						<div className="bg-green-50 border border-green-200 rounded-lg p-4">
							...
							{Number(formData.discount) > 0 && (
								<div className="text-sm text-gray-600 mt-2 text-right">
									Desconto aplicado: R$ {(formData.discountType === 'percentage'
										? (formItems.reduce(
											(sum, item) =>
												sum + calculateItemTotal(item.quantity, item.unitPrice),
											0,
