...
import type { PreSale, Customer, Product, PreSaleItem } from '../../../types';
import Button from '../../common/Button';
import InPageModal from '../../common/InPageModal';
import { Search, Plus, Eye, Edit, Trash2, Calculator } from 'lucide-react';
...
const PresalesPage: React.FC = () => {
	...
	const [statusFilter, setStatusFilter] = useState<PreSale['status'] | 'all'>('all');
	...
	const [formItems, setFormItems] = useState<Omit<PreSaleItem, 'id' | 'totalPrice'>[]>([]);
	...
	const filteredPreSales = preSales.filter(preSale => {
		const matchesSearch = preSale.customer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
			preSale.id.toLowerCase().includes(searchTerm.toLowerCase());
		const matchesStatus = statusFilter === 'all' || preSale.status === statusFilter;
	...
	const handleEditPreSale = (preSale: PreSale) => {
		...
		setFormData({
			...
			notes: preSale.notes || '',
			validUntil: preSale.validUntil ? preSale.validUntil.toISOString().split('T')[0] : '',
			discount: preSale.discount?.toString() || '',
			discountType: preSale.discountType || 'percentage',
		});
		setFormItems(preSale.items.map(item => ({
	...
	const updatePreSaleStatus = (newStatus: PreSale['status']) => {
		...
		setPreSales(prev => prev.map(preSale => 
	...
	const updateFormItem = (index: number, field: keyof Omit<PreSaleItem, 'id' | 'totalPrice'>, value: any) => {
	...
	const calculateFormTotal = () => {
		const itemsTotal = formItems.reduce((sum, item) => sum + calculateItemTotal(item.quantity, item.unitPrice), 0);
	...
	const handleSubmitForm = (e: React.FormEvent, isEdit = false) => {
		...
		const preSaleData: PreSale = {
			id: isEdit && selectedPreSale ? selectedPreSale.id : Date.now().toString(),
			...
			createdAt: isEdit && selectedPreSale ? selectedPreSale.createdAt : new Date(),
			updatedAt: new Date(),
		};
		...
		alert(isEdit ? 'Pré-venda atualizada com sucesso!' : 'Pré-venda criada com sucesso!');
	};
			...
			<div className="space-y-6">
					...
					<div className="flex items-center justify-between gap-4">
						<div className="flex items-center gap-4 flex-1">
							...
							<select
								value={statusFilter}
								onChange={(e) => setStatusFilter(e.target.value as PreSale['status'] | 'all')}
							...
							{filteredPreSales.map((preSale) => (
								...
								>
									<div className="flex justify-between items-start mb-3">
										...
										<span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(preSale.status)}`}>
									...
									<div className="flex justify-between items-center pt-3 border-t border-gray-200">
										...
										<div className="flex space-x-1">
											...
											{(preSale.status === 'draft' || preSale.status === 'pending') && (
												<button
													type="button"
						...
						{filteredPreSales.length === 0 && (
							<div className="text-center py-12">
								...
								<p className="text-gray-500 text-lg">
									{searchTerm ? 'Nenhuma pré-venda encontrada' : 'Nenhuma pré-venda cadastrada ainda.'}
								</p>
								{!searchTerm && (
									<Button
										variant="primary"
	...
	return (
		<div className="p-6">
						...
						<div className="bg-gray-50 p-4 rounded-lg">
							...
							<div className="grid grid-cols-2 gap-4 text-sm">
								...
								<div>
									...
									<p className="font-medium">{selectedPreSale.customer.email}</p>
								</div>
								<div>
									<span className="text-gray-600">Telefone:</span>
									<p className="font-medium">{selectedPreSale.customer.phone}</p>
								</div>
								<div>
								...
								<table className="w-full">
									<thead className="bg-gray-50">
										<tr>
											...
											<th className="px-4 py-2 text-right text-sm font-medium text-gray-700">Qtd</th>
											<th className="px-4 py-2 text-right text-sm font-medium text-gray-700">Preço Unit.</th>
							...
							<div className="mt-2 text-sm text-gray-600">
								...
								{selectedPreSale.validUntil && (
									<div className="flex justify-between mt-1">
										...
										<span>{selectedPreSale.validUntil.toLocaleDateString('pt-BR')}</span>
									</div>
								)}
					...
					<form onSubmit={handleSubmitForm} className="space-y-6">
						...
						<div>
							<div className="flex justify-between items-center mb-2">
								...
								<Button type="button" variant="secondary" onClick={addItemToForm}>
											...
											<select
												...
												onChange={(e) => {
													...
													if (product) {
														...
														updateFormItem(index, 'unitPrice', product.salePrice);
													}
												}}
			...
			{showEditModal && selectedPreSale && (
				...
				>
					<div className="px-6 py-6">
					<form onSubmit={(e) => handleSubmitForm(e, true)} className="space-y-6">
					...
					<div className="space-y-4">
						<div className="text-center">
							<p className="text-gray-600 mb-4">
								Status atual: <span className={`px-2 py-1 rounded text-sm font-medium ${getStatusColor(selectedPreSale.status)}`}>
									{getStatusLabel(selectedPreSale.status)}
								</span>
							</p>
						...
						<div className="grid grid-cols-1 gap-3">
							{(['pending', 'approved', 'cancelled', 'converted'] as PreSale['status'][]).map(status => (
								<button
									key={status}
