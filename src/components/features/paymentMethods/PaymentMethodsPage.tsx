...
const PaymentMethodsPage: React.FC = () => {
	...
	const handleDeleteConfirm = async () => {
		...
		setIsLoading(true);
		try {
			const success = await paymentMethodService.delete(confirmationDialog.paymentMethod.id);
			...
			// Show specific error message based on error type
			if (error instanceof Error) {
				if (error.message.includes('being used') || error.message.includes('sendo usada')) {
					toastService.error(TOAST_MESSAGES.paymentMethod.inUse);
				} else {
					toastService.error(error.message);
				}
			} else {
				toastService.error('Erro ao excluir forma de pagamento. Tente novamente.');
			}
		} finally {
			setIsLoading(false);
	...
	const handleEditSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (error instanceof Error) {
				if (error.message.includes('duplicate') || error.message.includes('já existe')) {
					toastService.error(TOAST_MESSAGES.paymentMethod.duplicateCode);
				} else if (error.message.includes('validation') || error.message.includes('inválidos')) {
					toastService.error(TOAST_MESSAGES.paymentMethod.invalidData);
				} else {
					toastService.error(error.message);
				}
			} else {
				toastService.error('Erro ao atualizar forma de pagamento. Tente novamente.');
			}
		} finally {
			setIsLoading(false);
		}
							...
							>
								<div className="flex justify-between items-start mb-2">
									...
									<div className="text-right flex-shrink-0">
										<span
											className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${paymentMethod.isActive
			...
			>
				<div className="space-y-4">
					...
					<p className="text-sm text-gray-500">
						Esta ação não pode ser desfeita. Se esta forma de pagamento estiver sendo
						usada em vendas, ela não poderá ser excluída.
					</p>
