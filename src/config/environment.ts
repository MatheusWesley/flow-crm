...
function getEnvEnvironment(key: string, defaultValue: EnvironmentConfig['appEnvironment']): EnvironmentConfig['appEnvironment'] {
    const value = import.meta.env[key];
...
function validateApiUrl(url: string): string {
    ...
    try {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
            console.warn(`Invalid API URL protocol: ${parsed.protocol}. Using default.`);
...
function createConfig(): EnvironmentConfig {
    const config: EnvironmentConfig = {
        apiBaseUrl: validateApiUrl(getEnvString('VITE_API_BASE_URL', DEFAULT_CONFIG.apiBaseUrl)),
        apiTimeout: getEnvNumber('VITE_API_TIMEOUT', DEFAULT_CONFIG.apiTimeout),
        tokenStorageKey: getEnvString('VITE_TOKEN_STORAGE_KEY', DEFAULT_CONFIG.tokenStorageKey),
        refreshTokenStorageKey: getEnvString('VITE_REFRESH_TOKEN_STORAGE_KEY', DEFAULT_CONFIG.refreshTokenStorageKey),
        tokenRefreshThreshold: getEnvNumber('VITE_TOKEN_REFRESH_THRESHOLD', DEFAULT_CONFIG.tokenRefreshThreshold),
        ...
        appEnvironment: getEnvEnvironment('VITE_APP_ENVIRONMENT', DEFAULT_CONFIG.appEnvironment),
        debugApiCalls: getEnvBoolean('VITE_DEBUG_API_CALLS', DEFAULT_CONFIG.debugApiCalls),
        debugAuth: getEnvBoolean('VITE_DEBUG_AUTH', DEFAULT_CONFIG.debugAuth),
        enableMockData: getEnvBoolean('VITE_ENABLE_MOCK_DATA', DEFAULT_CONFIG.enableMockData),
        enableOfflineMode: getEnvBoolean('VITE_ENABLE_OFFLINE_MODE', DEFAULT_CONFIG.enableOfflineMode),
        enableCsrfProtection: getEnvBoolean('VITE_ENABLE_CSRF_PROTECTION', DEFAULT_CONFIG.enableCsrfProtection),
