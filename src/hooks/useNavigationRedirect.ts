...
export const useNavigationRedirect = () => {
    ...
    const navigateToDefaultRoute = useCallback(() => {
        ...
        const defaultRoute = getDefaultRouteForUser(user.userType, user.permissions);
     ...
     */
    const canUserAccessRoute = useCallback((route: string) => {
     ...
     */
    const getAccessDeniedRedirect = useCallback((attemptedRoute: string) => {
