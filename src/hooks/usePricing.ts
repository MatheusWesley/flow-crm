...
export interface UsePricingReturn {
    calculateMarginMarkup: (purchasePrice: number, salePrice: number) => PriceCalculationData;
    calculateSuggestedPrices: (purchasePrice: number, targetMargin?: number) => PricingStrategies;
    calculatePriceFromMargin: (purchasePrice: number, marginPercentage: number) => number;
    calculatePriceFromMarkup: (purchasePrice: number, markupPercentage: number) => number;
...
export const usePricing = (): UsePricingReturn => {
     ...
     */
    const calculateMarginMarkup = useCallback((purchasePrice: number, salePrice: number): PriceCalculationData => {
        try {
            setError(null);
     ...
     */
    const calculateSuggestedPrices = useCallback((purchasePrice: number, targetMargin = 30): PricingStrategies => {
        try {
            setError(null);
            ...
            const competitive = calculatePriceFromMargin(purchasePrice, targetMargin - 5);
            const premium = calculatePriceFromMargin(purchasePrice, targetMargin + 10);
            const budget = calculatePriceFromMargin(purchasePrice, targetMargin - 10);
            ...
            const errorMessage = err instanceof Error ? err.message : 'Erro no cálculo de preços sugeridos';
            setError(errorMessage);
            throw new Error(errorMessage);
        }
    }, []);
     ...
     */
    const calculatePriceFromMargin = useCallback((purchasePrice: number, marginPercentage: number): number => {
        try {
            setError(null);
            ...
            const errorMessage = err instanceof Error ? err.message : 'Erro no cálculo por margem';
            setError(errorMessage);
            throw new Error(errorMessage);
        }
    }, []);
     ...
     */
    const calculatePriceFromMarkup = useCallback((purchasePrice: number, markupPercentage: number): number => {
        try {
            setError(null);
            ...
            const errorMessage = err instanceof Error ? err.message : 'Erro no cálculo por markup';
            setError(errorMessage);
            throw new Error(errorMessage);
        }
    }, []);
     ...
     */
    const getPriceSuggestions = useCallback((purchasePrice: number, targetMargin = 30) => {
        try {
            setIsCalculating(true);
            setError(null);

            const suggestions = calculateSuggestedPrices(purchasePrice, targetMargin);
            setPriceSuggestions(suggestions);
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : 'Erro ao calcular sugestões de preço';
            setError(errorMessage);
